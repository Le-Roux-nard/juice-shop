...
import {
  loadStaticChallengeData,
  loadStaticDeliveryData,
  loadStaticUserData,
  loadStaticSecurityQuestionsData,
} from "./staticData";
  ...
  await Promise.all(
    challenges.map(
      async ({
        ...
        const { enabled: isChallengeEnabled, disabledBecause } =
          utils.getChallengeEnablementStatus({
            disabledEnv: disabledEnv?.join(";") ?? "",
          } as ChallengeModel);
        ...
        const hasCodingChallenge =
          challengeKeysWithCodeChallenges.includes(key);
        ...
        try {
          datacache.challenges[key] = await ChallengeModel.create({
            ...
            description: isChallengeEnabled
              ? description
              : description +
                " <em>(This challenge is <strong>potentially harmful</strong> on " +
                disabledBecause +
                "!)</em>",
            difficulty,
            solved: false,
          ...
          logger.error(
            `Could not insert Challenge ${name}: ${utils.getErrorMessage(err)}`,
          );
        }
      },
    ),
...
async function createHints(ChallengeId: number, hints: string[]) {
  ...
  return await Promise.all(
    hints.map(async (hint) => {
      hint = hint.replace(
        /OWASP Juice Shop/,
        `${config.get<string>("application.name")}`,
      );
    ...
    users.map(
      async ({
        ...
        try {
          ...
          logger.error(
            `Could not insert User ${key}: ${utils.getErrorMessage(err)}`,
          );
        }
      },
    ),
  ...
  await Promise.all(
    deliveries.map(async ({ name, price, deluxePrice, eta, icon }) => {
      try {
        ...
        logger.error(
          `Could not insert Delivery Method: ${utils.getErrorMessage(err)}`,
        );
      }
    }),
  );
...
async function deleteUser(userId: number) {
  return await UserModel.destroy({ where: { id: userId } }).catch(
    (err: unknown) => {
      logger.error(
        `Could not perform soft delete for the user ${userId}: ${utils.getErrorMessage(err)}`,
      );
    },
  );
}
...
async function deleteProduct(productId: number) {
  return await ProductModel.destroy({ where: { id: productId } }).catch(
    (err: unknown) => {
      logger.error(
        `Could not perform soft delete for the product ${productId}: ${utils.getErrorMessage(err)}`,
      );
    },
  );
}
...
async function createRandomFakeUsers() {
  ...
  return await Promise.all(
    new Array(config.get("application.numberOfRandomFakeUsers")).fill(0).map(
      async () =>
  ...
  return await Promise.all(
    config.get<ProductConfig[]>("products").map(async (product, index) => {
      return await QuantityModel.create({
        ...
        logger.error(
          `Could not create quantity: ${utils.getErrorMessage(err)}`,
        );
      });
    }),
  );
  ...
  const memories = [
    ...
    ...structuredClone(config.get<MemoryConfig[]>("memories")).map(
      async (memory) => {
        ...
        const userIdOfMemory =
          datacache.users[memory.user].id.valueOf() ?? null;
        if (!userIdOfMemory) {
          logger.warn(
            `Could not find saved user for memory ${memory.caption}!`,
          );
          return;
        }
        ...
        return await MemoryModel.create({
          ...
          logger.error(
            `Could not create memory: ${utils.getErrorMessage(err)}`,
          );
        });
      },
    ),
...
async function createProducts() {
  const products = structuredClone(config.get<ProductConfig[]>("products")).map(
    (product) => {
  ...
  const tamperingChallengeProduct = products.find(
    ({ urlForProductTamperingChallenge }) => urlForProductTamperingChallenge,
  );
    ...
    products.map(
      async ({
        ...
        await ProductModel.create({
          ...
          .catch((err: unknown) => {
            logger.error(
              `Could not insert Product ${product.name}: ${utils.getErrorMessage(err)}`,
            );
          })
              ...
              await Promise.all(
                reviews.map(({ text, author }) =>
                  reviewsCollection
                    ...
                    .catch((err: unknown) => {
                      logger.error(
                        `Could not insert Product Review ${text}: ${utils.getErrorMessage(err)}`,
                      );
                    }),
                ),
              ),
  ...
  return await Promise.all(
    baskets.map(async (basket) => {
      return await BasketModel.create({
        ...
        logger.error(
          `Could not insert Basket for UserId ${basket.UserId}: ${utils.getErrorMessage(err)}`,
        );
      });
    }),
  );
  ...
  return await Promise.all(
    basketItems.map(async (basketItem) => {
      return await BasketItemModel.create(basketItem).catch((err: unknown) => {
        logger.error(
          `Could not insert BasketItem for BasketId ${basketItem.BasketId}: ${utils.getErrorMessage(err)}`,
        );
      });
    }),
  );
...
async function createAnonymousFeedback() {
  ...
  return await Promise.all(
    feedbacks.map(
      async (feedback) =>
        await createFeedback(null, feedback.comment, feedback.rating),
    ),
  );
}
...
async function createFeedback(
  ...
  return await FeedbackModel.create({
    ...
    logger.error(
      `Could not insert Feedback ${authoredComment} mapped to UserId ${UserId}: ${utils.getErrorMessage(err)}`,
    );
  });
}
...
async function createRecycle(data: {
  ...
  return await RecycleModel.create({
    ...
    logger.error(
      `Could not insert Recycling Model: ${utils.getErrorMessage(err)}`,
    );
  });
}
  ...
  await Promise.all(
    questions.map(async ({ question }) => {
      try {
        ...
        logger.error(
          `Could not insert SecurityQuestion ${question}: ${utils.getErrorMessage(err)}`,
        );
      }
    }),
  );
...
async function createSecurityAnswer(
  UserId: number,
  ...
  return await SecurityAnswerModel.create({
    ...
    logger.error(
      `Could not insert SecurityAnswer ${answer} mapped to UserId ${UserId}: ${utils.getErrorMessage(err)}`,
    );
  });
}
    ...
    orders.map(
      ({ orderId, email, totalPrice, bonus, products, eta, delivered }) =>
        ordersCollection
          ...
          .catch((err: unknown) => {
            logger.error(
              `Could not insert Order ${orderId}: ${utils.getErrorMessage(err)}`,
            );
          }),
    ),
  );
