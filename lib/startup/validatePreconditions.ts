  ...
  const asyncConditions = (
    await Promise.all([
      ...
      checkIfPortIsAvailable(
        process.env.PORT ?? config.get<number>("server.port"),
      ),
...
export const checkIfRunningOnSupportedNodeVersion = (
  ...
  if (!effectiveVersionRange) {
    logger.warn(
      `Invalid Node.js version range ${colors.bold(supportedVersion)} in package.json (${colors.red("NOT OK")})`,
    );
    return false;
  }
  if (!semver.satisfies(runningVersion, effectiveVersionRange)) {
    logger.warn(
      `Detected Node version ${colors.bold(runningVersion)} is not in the supported version range of ${supportedVersion} (${colors.red("NOT OK")})`,
    );
    return false;
  }
  logger.info(
    `Detected Node.js version ${colors.bold(runningVersion)} (${colors.green("OK")})`,
  );
  return true;
};
...
export const checkIfRunningOnSupportedOS = (runningOS: string) => {
  ...
  if (!supportedOS.includes(runningOS)) {
    logger.warn(
      `Detected OS ${colors.bold(runningOS)} is not in the list of supported platforms ${supportedOS.toString()} (${colors.red("NOT OK")})`,
    );
    return false;
  }
...
export const checkIfRunningOnSupportedCPU = (runningArch: string) => {
  ...
  if (!supportedArch.includes(runningArch)) {
    logger.warn(
      `Detected CPU ${colors.bold(runningArch)} is not in the list of supported architectures ${supportedArch.toString()} (${colors.red("NOT OK")})`,
    );
    return false;
  }
  logger.info(
    `Detected CPU ${colors.bold(runningArch)} (${colors.green("OK")})`,
  );
  return true;
};
...
export const checkIfDomainReachable = async (domain: string) => {
  return checkInternetConnected({ domain })
    .then(() => {
      logger.info(
        `Domain ${colors.bold(domain)} is reachable (${colors.green("OK")})`,
      );
      return true;
    })
    .catch(() => {
      logger.warn(
        `Domain ${colors.bold(domain)} is not reachable (${colors.yellow("NOT OK")} in a future major release)`,
      );
      // @ts-expect-error FIXME Type problem by accessing key via variable
      domainDependencies[domain].forEach((dependency: string) => {
        logger.warn(
          `${colors.italic(dependency)} will not work as intended without access to ${colors.bold(domain)}`,
        );
      });
...
export const checkIfPortIsAvailable = async (port: number | string) => {
  ...
  return await new Promise((resolve, reject) => {
    portscanner.checkPortStatus(
      portNumber,
      function (error: unknown, status: string) {
        if (error) {
          reject(error);
        } else {
          if (status === "open") {
            logger.warn(
              `Port ${colors.bold(port.toString())} is in use (${colors.red("NOT OK")})`,
            );
            resolve(false);
          } else {
            logger.info(
              `Port ${colors.bold(port.toString())} is available (${colors.green("OK")})`,
            );
            resolve(true);
          }
        }
...
export const checkIfRequiredFileExists = async (
  pathRelativeToProjectRoot: string,
) => {
  const fileName = pathRelativeToProjectRoot.substr(
    pathRelativeToProjectRoot.lastIndexOf("/") + 1,
  );

  return await access(path.resolve(pathRelativeToProjectRoot))
    .then(() => {
      logger.info(
        `Required file ${colors.bold(fileName)} is present (${colors.green("OK")})`,
      );
      return true;
    })
    .catch(() => {
      logger.warn(
        `Required file ${colors.bold(fileName)} is missing (${colors.red("NOT OK")})`,
      );
      return false;
    });
};
