...
const validateDependencies = async ({
  ...
  try {
    ...
    logger.warn(
      `Dependencies in ${colors.bold(packageDir + "/package.json")} could not be checked due to "${utils.getErrorMessage(err)}" error (${colors.red("NOT OK")})`,
    );
  }

  if (dependencies.depsWereOk === true) {
    logger.info(
      `All dependencies in ${colors.bold(packageDir + "/package.json")} are satisfied (${colors.green("OK")})`,
    );
  } else {
    logger.warn(
      `Dependencies in ${colors.bold(packageDir + "/package.json")} are not rightly satisfied (${colors.red("NOT OK")})`,
    );
    success = false;
