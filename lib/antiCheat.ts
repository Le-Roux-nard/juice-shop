...
const coupledChallenges = {
  // TODO prevent also near-identical challenges (e.g. all null byte file access or dom xss + bonus payload etc.) from counting as cheating
...
]; // seed with server start timestamp

const preSolveInteractions: Array<{
...
export const checkForPreSolveInteractions =
  () =>
...
export const calculateCheatScore = (challenge: Challenge) => {
  ...
  timeFactor *=
    challenge.tutorialOrder && config.get("hackingInstructor.isEnabled")
      ? 0.5
      : 1;
  ...
  const minutesSincePreviousSolve =
    (timestamp.getTime() - previous().timestamp.getTime()) / 60000;
  cheatScore += Math.max(
    0,
    1 - minutesSincePreviousSolve / minutesExpectedToSolve,
  );

  const preSolveInteraction = preSolveInteractions.find(
    (preSolveInteraction) => preSolveInteraction.challengeKey === challenge.key,
  );
  let percentPrecedingInteraction = -1;
  if (preSolveInteraction) {
    percentPrecedingInteraction =
      preSolveInteraction.interactions.filter(Boolean).length /
      preSolveInteraction.interactions.length;
    const multiplierForMissingExpectedInteraction =
      1 + Math.max(0, 1 - percentPrecedingInteraction) / 2;
    cheatScore *= multiplierForMissingExpectedInteraction;
    cheatScore = Math.min(1, cheatScore);
  }

  logger.info(
    `Cheat score for ${areCoupled(challenge, previous().challenge) ? "coupled " : isTrivial(challenge) ? "trivial " : ""}${challenge.tutorialOrder ? "tutorial " : ""}${colors.cyan(challenge.key)} solved in ${Math.round(minutesSincePreviousSolve)}min (expected ~${minutesExpectedToSolve}min) with${config.get("challenges.showHints") ? "" : "out"} hints allowed${percentPrecedingInteraction > -1 ? " and " + percentPrecedingInteraction * 100 + "% expected preceding URL interaction" : ""}: ${cheatScore < 0.33 ? colors.green(cheatScore.toString()) : cheatScore < 0.66 ? colors.yellow(cheatScore.toString()) : colors.red(cheatScore.toString())}`,
  );
...
export const calculateFindItCheatScore = async (challenge: Challenge) => {
  ...
  timeFactor *=
    ...
    config.get("hackingInstructor.isEnabled")
      ? 0.5
      : 1;
  let cheatScore = 0;
  ...
  logger.info(
    `Cheat score for "Find it" phase of ${challenge.key === "scoreBoardChallenge" && config.get("hackingInstructor.isEnabled") ? "tutorial " : ""}${colors.cyan(challenge.key)} solved in ${Math.round(minutesSincePreviousSolve)}min (expected ~${minutesExpectedToSolve}min): ${cheatScore < 0.33 ? colors.green(cheatScore.toString()) : cheatScore < 0.66 ? colors.yellow(cheatScore.toString()) : colors.red(cheatScore.toString())}`,
  );
...
export const calculateFixItCheatScore = async (challenge: Challenge) => {
  ...
  logger.info(
    `Cheat score for "Fix it" phase of ${colors.cyan(challenge.key)} solved in ${Math.round(minutesSincePreviousSolve)}min (expected ~${minutesExpectedToSolve}min): ${cheatScore < 0.33 ? colors.green(cheatScore.toString()) : cheatScore < 0.66 ? colors.yellow(cheatScore.toString()) : colors.red(cheatScore.toString())}`,
  );
...
export const totalCheatScore = () => {
  return solves.length > 1
    ? median(solves.map(({ cheatScore }) => cheatScore))
    : 0;
};
...
function areCoupled(challenge: Challenge, previousChallenge: Challenge) {
  ...
  return (
    coupledChallenges[challenge.key]?.indexOf(previousChallenge.key) > -1 ||
    coupledChallenges[previousChallenge.key]?.indexOf(challenge.key) > -1
  );
}
...
const checkForIdenticalSolvedChallenge = async (
  challenge: Challenge,
): Promise<boolean> => {
