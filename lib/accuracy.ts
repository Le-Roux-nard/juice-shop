...
export const storeFindItVerdict = (
...
export const storeFixItVerdict = (
  challengeKey: ChallengeKey,
  verdict: boolean,
) => {
...
function calculateAccuracy(challengeKey: ChallengeKey, phase: Phase) {
  ...
  logger.info(
    `Accuracy for '${phase === "fix it" ? "Fix It" : "Find It"}' phase of coding challenge ${colors.cyan(challengeKey)}: ${accuracy > 0.5 ? colors.green(accuracy.toString()) : accuracy > 0.25 ? colors.yellow(accuracy.toString()) : colors.red(accuracy.toString())}`,
  );
  return accuracy;
}
