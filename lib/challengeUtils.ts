    ...
    .then(
      async (solvedChallenge: {
        ...
        if (!isRestore) {
          ...
          const hintsAvailable = await HintModel.count({
            where: { ChallengeId: solvedChallenge.id },
          });
          const hintsUnlocked = await HintModel.count({
            where: { ChallengeId: solvedChallenge.id, unlocked: true },
          });
...
export const sendNotification = function (
  ...
  if (!notSolved(challenge)) {
    ...
    const wasPreviouslyShown = notifications.some(
      ({ key }) => key === challenge.key,
    );
...
export const solveFindIt = async function (
  key: ChallengeKey,
  isRestore: boolean = false,
) {
  ...
  logger.info(
    `${isRestore ? colors.grey("Restored") : colors.green("Solved")} 'Find It' phase of coding challenge ${colors.cyan(solvedChallenge.key)} (${solvedChallenge.name})`,
  );
  if (!isRestore) {
...
export const solveFixIt = async function (
  key: ChallengeKey,
  isRestore: boolean = false,
) {
  const solvedChallenge = challenges[key];
  ...
  logger.info(
    `${isRestore ? colors.grey("Restored") : colors.green("Solved")} 'Fix It' phase of coding challenge ${colors.cyan(solvedChallenge.key)} (${solvedChallenge.name})`,
  );
  if (!isRestore) {
