...
export function serveMetrics() {
  return async (req: Request, res: Response, next: NextFunction) => {
    challengeUtils.solveIf(challenges.exposedMetricsChallenge, () => {
      ...
      const ignoredUserAgents = config.get<string[]>(
        "challenges.metricsIgnoredUserAgents",
      );
    ...
    setInterval(() => {
      try {
        ...
        for (const {
          ...
          category,
          solved,
        } of Object.values<ChallengeModel>(challenges)) {
        ...
        void retrieveChallengesWithCodeSnippet().then((challenges) => {
          ChallengeModel.count({
            ...
            .catch(() => {
              throw new Error(
                "Unable to retrieve and count such challenges. Please try again",
              );
            });
        ...
        logger.warn(
          "Error during metrics update loop: + " + utils.getErrorMessage(e),
        );
      }
    }, 5000);
