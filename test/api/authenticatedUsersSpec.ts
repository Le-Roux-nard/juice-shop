...
const authHeader = {
  Authorization: `Bearer ${security.authorize({ data: { email: "admin@juice-sh.op" } })}`,
...
describe("/rest/user/authentication-details", () => {
  it("GET all users with password replaced by asterisks", () => {
    return frisby
      .get(`${REST_URL}/user/authentication-details`, { headers: authHeader })
      .expect("status", 200)
      .expect("json", "data.?", {
        password: "********************************",
      });
  ...
  it("GET returns lastLoginTime for users with active sessions", async () => {
    ...
    const jim = response.json.data.find((user: any) =>
      user.email.startsWith("jim@"),
    );
